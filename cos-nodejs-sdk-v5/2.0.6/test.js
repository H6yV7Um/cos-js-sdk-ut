    ...
    it('正常分片上传 object', function (done) {
        ...
        var put = function () {
            ...
            cos.sliceUploadFile({
                ...
                assert.equal(true, data && data.ETag.length > 0 && lastPercent === 1);
                fs.unlinkSync(filepath);
                done();
            });
        };
...
describe('BucketACL', function () {
    ...
    it('putBucketAcl() header ACL:public-read', function (done) {
        cos.putBucketAcl({
            ...
            assert(!err);
            cos.getBucketAcl({Bucket: config.Bucket, Region: config.Region}, function (err, data) {
                assert.ok(data.Grants[1] && data.Grants[1].Permission === 'READ', '设置 ACL Permission 正确');
                done();
            });
        });
    });
    it('putBucketAcl() header ACL:public-read-write', function (done) {
        cos.putBucketAcl({
            ...
            assert(!err);
            cos.getBucketAcl({Bucket: config.Bucket, Region: config.Region}, function (err, data) {
                assert.ok(data.Grants[1] && data.Grants[1].Permission === 'READ', '设置 ACL Permission Public-read正确');
                assert.ok(data.Grants[2] && data.Grants[2].Permission === 'WRITE', '设置 ACL Permission Public-write正确');
                done();
            });
        });
    });
    ...
    it('putBucketAcl() header ACL:public-read, GrantFullControl:uin="1001", uin="1002"', function (done) {
        ...
        cos.putBucketAcl({
            ...
            cos.getBucketAcl({Bucket: config.Bucket, Region: config.Region}, function (err, data) {
                assert.ok(data.GrantFullControl = GrantFullControl);
                assert.ok(data.Grants[0] && data.Grants[0].Permission === 'READ', '设置 ACL Permission Public-read正确');
                done();
            });
        });
    });
...
describe('BucketPolicy', function () {
    var shortRegion = config.Region.split('.')[1];
    ...
    var Policy = {
        ...
        "statement": [{
            ...
            "resource": ["qcs::cos:" + shortRegion + ":uid/" + AppId + ":" + Bucket + "-" + AppId + "." + config.Region + ".myqcloud.com//" + AppId + "/" + Bucket + "/" + Prefix + "/*"] // 1250000000 是 appid
        }]
    };
...
describe('BucketLocation', function () {
    it('getBucketLocation()', function (done) {
        cos.getBucketLocation({
            ...
            assert('cos.' + data.LocationConstraint === config.Region);
            done();
        });
    });
});
...
describe('BucketLifecycle', function () {
    ...
    it('putBucketLifecycle() multi', function (done) {
        RulesMulti[0].Filter.Prefix = 'test_' + Date.now().toString(36);
        cos.putBucketLifecycle({
            Bucket: config.Bucket,
            Region: config.Region,
            LifecycleConfiguration: {
